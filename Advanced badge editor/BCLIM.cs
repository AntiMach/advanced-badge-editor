...
using Advanced_badge_editor.Properties; // For other projects, replace this line with the namespace that contains your ETC1Lib.dll resource.
...
namespace CTR
{
    ...
    /// <param name="y">Output Y coordinate</param>
        ...
        public static CLIM newFromArray(byte[] data)
        {
            CLIM bclim = new CLIM();
            byte[] byteArray = data;
            using (BinaryReader br = new BinaryReader(new MemoryStream(byteArray)))
...
namespace CTR
{
    class Badge
    {
        public struct BADGE_IMG
        {
            public BCLIM.CLIM Image64;
            public BCLIM.CLIM Shape64;
            public BCLIM.CLIM Image32;
            public BCLIM.CLIM Shape32;
        }

        public static Bitmap StretchImage(Bitmap img)
        {
        ...
        public static Bitmap[] BADGEtoBMP(BADGE_IMG badge)
        {
            Bitmap Image64 = BCLIM.getIMG(badge.Image64);
            Bitmap Shape64 = BCLIM.getIMG(badge.Shape64);
            Bitmap Overlay64 = new Bitmap(64, 64);

            for (int i = 0; i < Overlay64.Width; i++)
            {
                for (int ii = 0; ii < Overlay64.Height; ii++)
                {
                    if (Shape64.GetPixel(i, ii).A == 255)
                        Overlay64.SetPixel(i, ii, Image64.GetPixel(i, ii));
                }
            }

            Bitmap Image32 = BCLIM.getIMG(badge.Image32);
            Bitmap Shape32 = BCLIM.getIMG(badge.Shape32);
            Bitmap Overlay32 = new Bitmap(32, 32);

            for (int i = 0; i < Overlay32.Width; i++)
            {
                for (int ii = 0; ii < Overlay32.Height; ii++)
                {
                    if (Shape32.GetPixel(i, ii).A == 255)
                        Overlay32.SetPixel(i, ii, Image32.GetPixel(i, ii));
                }
            }

            Bitmap[] Overlays = { Overlay64, Overlay32};
            return Overlays;
        }
        /// <summary>
        /// Creates a badge from a bitmap
        /// </summary>
        ...
        public static BADGE_IMG BMPtoBADGE(Bitmap img, bool PixelBadge)
        {
            ...
            Output.Shape32 = BCLIM.newFromArray(BCLIM.IMGToBCLIM(Shape32, 'd'));
            return Output;
        }

        public static Bitmap downscaleImg(Bitmap img, int dscale, bool hq)
        {
        ...
        public static BCLIM.CLIM adjustForSet(Bitmap img)
        {
            Bitmap Output = new Bitmap(64, 64);
            Graphics Adjust = Graphics.FromImage(Output);

            Adjust.DrawImage(img, new Rectangle(0, 0, 48, 48));
            Output = StretchImage(Output);

            BCLIM.CLIM clim = BCLIM.newFromArray(BCLIM.IMGToBCLIM(Output, '5'));

            return clim;
        }

        public static Bitmap cropImage(Bitmap img, Rectangle area)
        {
